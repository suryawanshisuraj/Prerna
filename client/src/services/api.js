// API service layer with built-in instant fallback for Vercel static & serverless deployments

const API_BASE = '/api';

const LOCAL_JOURNAL_DATA = {
  info: {
    title: "Digital Love Girlfriend Day Journal",
    theme: "Ethereal Devotion",
    couple: { from: "Suraj", to: "Prerna" },
  ...
  timeline: [
    ...
    },
    {
      ...
      type: "collage",
      memories: [
        { id: "m1", caption: "Cozy Vintage Selfie", rotation: "3deg", image: "/photo7.jpg" },
        { id: "m2", caption: "Matching Navy Suits", rotation: "-4deg", image: "/photo9.jpg" },
        { id: "m3", caption: "Art Festival Hugs", rotation: "2deg", image: "/photo10.jpg" }
      ]
    }
  ],
  gallery: [
    { id: "g1", caption: "Sunny Smiles", rotation: "-2deg", prompt: "Suraj & Prerna smiling together in natural sunlight.", image: "/photo1.jpg" },
    { id: "g2", caption: "Traditional Festive Glow", rotation: "1.5deg", prompt: "Prerna in her beautiful floral pink ethnic outfit and silver jhumkas next to Suraj.", image: "/photo3.jpg" },
    { id: "g3", caption: "Sunny Park Days", rotation: "-1deg", prompt: "Suraj & Prerna sitting together under shady trees on a bright afternoon.", image: "/photo4.jpg" },
    { id: "g4", caption: "City Night Out", rotation: "2deg", prompt: "Suraj & Prerna dressed up together for a fun night out in the city.", image: "/photo6.jpg" },
    { id: "g5", caption: "Sky Blue & Silver Grace", rotation: "-1.5deg", prompt: "Prerna in a metallic blue saree and Suraj in sky blue kurta.", image: "/photo12.jpg" },
    { id: "g6", caption: "Park Fountain Moments", rotation: "1deg", prompt: "Selfie in front of the park fountain.", image: "/photo13.jpg" }
  ],
  reasons: [
...
export async function fetchJournalInfo() {
  try {
    ...
    return json.data || LOCAL_JOURNAL_DATA.info;
  } catch (e) {
    return LOCAL_JOURNAL_DATA.info;
  }
}
...
export async function fetchTimeline() {
  try {
    ...
    if (!res.ok) throw new Error('API request failed');
    const json = await res.json();
    return json.data || LOCAL_JOURNAL_DATA.timeline;
  } catch (e) {
    return LOCAL_JOURNAL_DATA.timeline;
  }
}
...
export async function fetchGallery() {
  try {
    ...
    if (!res.ok) throw new Error('API request failed');
    const json = await res.json();
    return json.data || LOCAL_JOURNAL_DATA.gallery;
  } catch (e) {
    return LOCAL_JOURNAL_DATA.gallery;
  }
}
...
export async function fetchReasons() {
  try {
    ...
    if (!res.ok) throw new Error('API request failed');
    const json = await res.json();
    return json.data || LOCAL_JOURNAL_DATA.reasons;
  } catch (e) {
    return LOCAL_JOURNAL_DATA.reasons;
  }
}
...
export async function fetchQuiz() {
  try {
    ...
    if (!res.ok) throw new Error('API request failed');
    const json = await res.json();
    return (json.data && json.data.length > 0) ? json.data : LOCAL_JOURNAL_DATA.quiz;
  } catch (e) {
    return LOCAL_JOURNAL_DATA.quiz;
  }
}
